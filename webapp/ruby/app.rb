...
SQL
  ...
  get '/diary/entries/:account_name' do
    ...
    owner = user_from_account(params['account_name'])
    query = if permitted?(owner[:id])
              'SELECT entries.*,x.cc FROM entries JOIN (SELECT id,COUNT(*) AS cc FROM comments) AS x ON entries.id = x.id WHERE user_id = ? ORDER BY created_at DESC LIMIT 20'
            else
              'SELECT entries.*,x.cc FROM entries JOIN (SELECT id,COUNT(*) AS cc FROM comments) AS x ON entries.id = x.id WHERE user_id = ? AND private=0 ORDER BY created_at DESC LIMIT 20'
            end
    entries = db.xquery(query, owner[:id])
      .map{ |entry| entry[:is_private] = (entry[:private] == 1); entry[:title], entry[:content] = entry[:body].split(/\n/, 2); entry }
    mark_footprint(owner[:id])
